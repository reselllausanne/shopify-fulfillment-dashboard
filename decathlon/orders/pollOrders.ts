...
export type DecathlonOrdersPollOptions = {
  state?: string;
  limit?: number;
  maxPages?: number;
};

export type DecathlonOrdersPollResult = {
  ok: boolean;
  fetched: number;
  upserted: number;
  returns: Awaited<ReturnType<typeof syncDecathlonReturns>>;
  inventory: {
    applied: number;
...
export async function runDecathlonOrdersPoll(
  options: DecathlonOrdersPollOptions = {}
): Promise<DecathlonOrdersPollResult> {
  try {
    const prismaAny = prisma as any;
    ...
    return {
      ...
      fetched: 0,
      upserted: 0,
      returns: { fetched: 0, upserted: 0, lines: 0, restocked: 0 },
      inventory: { applied: 0, alreadyProcessed: 0, unresolved: 0, invalid: 0 },
