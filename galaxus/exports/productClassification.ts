...
//
// Category paths: galaxus/exports/galaxusCategoryPaths.ts (verify vs Partner Portal export).

import {
  type GalaxusProductKind,
  galaxusCategoryPathForKind,
  defaultGalaxusProductKind,
...
type ClassificationInput = {
  ...
  brand?: string | null;
  sizeRaw?: string | null;
  /** stx | ner | wel | … — drives WEL defaults and supplier-specific rules. */
  supplierKey?: string | null;
  /** WellPlayed Shopify product_type (WEL scraper). */
...
function combinedText(input: ClassificationInput): string {
  ...
  const parts = [
    ...
    sanitizeText(input.supplierProductType ?? ""),
    breadcrumbText,
  ];
...
const FOOTWEAR_RE =
  /\b(sneaker|samba|gazelle|campus|superstar|jordan|dunk|air ?max|yeezy|asics|new ?balance|salomon|shoe|shoes|trainer)\b/i;
...
const TROUSERS_RE = /\b(trousers?|jeans|sweatpants?|joggers?|cargo pants|chinos?|\bpant\b|\bpants\b)\b/i;
const UNDERWEAR_RE = /\b(boxer|boxer briefs|underwear|briefs|trunks)\b/i;
const BOOTS_RE = /\b(boots?|neumel|timberland)\b/i;
const SLIPPERS_RE = /\b(slipper|slippers|tazz|tasman|clog|mule|hausschuh)\b/i;
const SANDALS_RE = /\b(sandal|sandals|slide|slides)\b/i;
...
/** UGG seasonal footwear — STX/NER via StockX, not generic sneakers path. */
function classifyUggFootwearKind(title?: string | null): GalaxusProductKind {
  const text = sanitizeText(title).toLowerCase();
  if (!text) return "boots";
...
export function classifyGalaxusProductKind(input: ClassificationInput): GalaxusProductKind {
  const supplierKey = String(input.supplierKey ?? "").toLowerCase();
  ...
  if (brand === "ugg") return classifyUggFootwearKind(input.title);
...
export function resolveGalaxusProductCategoryPath(input: ClassificationInput): string {
  ...
  return galaxusCategoryPathForKind(kind, input.supplierKey);
}
...
export function resolveGalaxusDescription(input: {
  ...
  if (kind === "boardgame" || kind === "cardgame" || kind === "rpg") {
    return `${prefix}. Tabletop game for players and collectors, sealed in original packaging.`;
  }
  if (kind === "miniature") return `${prefix}. Miniatures game product for hobbyists and collectors.`;
  if (kind === "game_accessory" || kind === "dice" || kind === "playmat") {
    return `${prefix}. Game accessory designed for tabletop and card games.`;
  }
  if (kind === "tradingcard") {
    return `${prefix}. Trading card product for collectors, sealed in original packaging.`;
  }
  if (kind === "slippers" || kind === "boots" || kind === "sandals") {
    return `${prefix}. Comfortable footwear with durable construction for everyday wear.`;
  }
  if (kind === "shorts") return `${prefix}. Everyday shorts with comfortable fit and versatile styling.`;
  if (kind === "apparel") return `${prefix}. Casual apparel piece designed for daily wear and comfort.`;
  if (kind === "backpack") {
    return `${prefix}. Functional backpack with durable construction and organized compartments for everyday carry.`;
  }
  if (kind === "bag") return `${prefix}. Versatile bag designed for everyday use with practical storage and durable materials.`;
  if (kind === "phone") return `${prefix}. Smartphone accessory designed to support focus and reduce distractions.`;
  if (kind === "pool_robot") {
  ...
  if (kind === "camera") return `${prefix}. Digital camera capturing high-resolution photos and video with versatile shooting modes.`;
  if (kind === "tumbler") {
    return `${prefix}. Insulated tumbler keeping drinks cold for hours, designed for everyday hydration.`;
  }
  if (kind === "watch") return `${prefix}. Wristwatch with distinctive design and reliable timekeeping for everyday wear.`;
  if (kind === "lego") return `${prefix}. LEGO construction set encouraging creative building and imaginative play.`;
  if (kind === "cap" || kind === "hat") {
    return `${prefix}. Headwear with adjustable fit and durable construction for everyday wear.`;
  }
  if (kind === "socks") return `${prefix}. Socks designed for everyday comfort and durable wear.`;
  if (kind === "trousers") return `${prefix}. Trousers designed for everyday wear with comfortable fit and durable fabric.`;
  if (kind === "underwear") return `${prefix}. Underwear designed for everyday comfort with soft, breathable fabric.`;
  if (kind === "sticker") return `${prefix}. Sticker set for customisation and personal expression.`;
  if (kind === "skateboard") return `${prefix}. Skateboard deck and accessories built for performance and durability.`;
  if (kind === "charger") return `${prefix}. Charging accessory delivering reliable power to compatible devices.`;
  if (kind === "headphone") {
    return `${prefix}. Headphones delivering high-quality audio with comfortable fit for everyday listening.`;
  }
  if (kind === "console") return `${prefix}. Gaming console delivering high-performance gameplay and entertainment.`;
