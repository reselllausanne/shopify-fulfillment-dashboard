...
const globalForRateLimit = globalThis as typeof globalThis & {
  __shopifyReturnablesRateLimit?: Map<string, number[]>;
};
if (!globalForRateLimit.__shopifyReturnablesRateLimit) {
  globalForRateLimit.__shopifyReturnablesRateLimit = new Map<string, number[]>();
}
const rateLimitStore = globalForRateLimit.__shopifyReturnablesRateLimit;
...
function ensureOriginAllowed(origin: string | null) {
  ...
  const allowed = resolveAllowedOrigins();
  if (!allowed.includes(origin)) {
    throw new ShopifyReturnRequestError("FORBIDDEN_ORIGIN", "Origin is not allowed", 403);
  }
}
...
function ensurePublicApiKey(request: Request) {
  ...
  if (!provided || provided !== required) {
    throw new ShopifyReturnRequestError("UNAUTHORIZED", "Missing or invalid returns API key", 401);
  }
}
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const result = await getReturnableItemsForOrder({
    ...
    console.error("[SHOPIFY][RETURNS][RETURNABLE] Unhandled error", error);
    return NextResponse.json(
      {
        success: false,
        error: "SERVER_ERROR",
