...
export async function POST(req: Request) {
  ...
  const minSales = Math.min(Math.max(Number(searchParams.get("minSales") ?? 3), 0), 1000);
  const limit = Math.min(Math.max(Number(searchParams.get("limit") ?? 100), 1), 5000);
  // mode=sales   -> legacy behaviour, rank by sales_count_15_days (default)
  // mode=activity-> rank by KickDBProduct.updatedAt (most recent SSE refresh
  //                 wins; catches live-priced products with 0 stored sales)
  // mode=all     -> no filter, oldest-first (drain remaining untracked catalog)
  const mode = (searchParams.get("mode") ?? "sales").toLowerCase();

  try {
    let flagged: Array<{ kickdbProductId: string; priorityScore: number }> = [];

    if (mode === "activity") {
      flagged = await prisma.$queryRaw`
        INSERT INTO "public"."ShopifySyncState"
          ("id", "kickdbProductId", "syncStatus", "priorityScore", "createdAt", "updatedAt")
        SELECT gen_random_uuid(),
               p."kickdbProductId",
               'create_candidate',
               EXTRACT(EPOCH FROM p."updatedAt")::int,
               NOW(), NOW()
