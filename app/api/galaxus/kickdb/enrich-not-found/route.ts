...
export async function POST() {
  try {
    ...
    await prismaAny.galaxusJobRun.create({
      data: {
        jobName: "kickdb-enrich-not-found",
        startedAt,
        finishedAt: new Date(),
        success: true,
        resultJson: payload,
    ...
    console.error("[GALAXUS][KICKDB][ENRICH-NOT-FOUND] Failed:", error);
    const prismaAny = prisma as any;
    await prismaAny.galaxusJobRun.create({
      data: {
        jobName: "kickdb-enrich-not-found",
        ...
        finishedAt: new Date(),
        success: false,
        errorMessage: error?.message ?? "Failed to enrich NOT_FOUND",
      },
    });
    return NextResponse.json({ ok: false, error: error?.message ?? "Failed to enrich NOT_FOUND" }, { status: 500 });
  }
}
