...
/**
 * Channel-agnostic StockX buy lookup by Pro order # (Shopify manual entry, etc.).
 * Walks pasted bearer + all known server account tokens (same as Galaxus lookup).
 */
  ...
  try {
    ...
    if (!looksLikeStockxOrderNumber(orderNumber)) {
      return NextResponse.json(
        { ok: false, error: "Not a StockX order number format" },
        { status: 400 }
      );
    }
    ...
    const fields = applyStockxDetailsToDecathlonMatchFields(resolved.listNode, resolved.details, {
      matchReasons: ["MANUAL_STOCKX_ORDER_LOOKUP"],
    });
