...
const NAV_GROUPS: NavGroup[] = [
  {
    ...
    links: [
      ...
      { label: "Stock review", href: "/supplier-stock", badge: "new" },
    ],
  },
  {
    label: "Health",
    links: [
      { label: "Today", href: "/health", badge: "new" },
      { label: "Ops / WHOOP", href: "/health/ops" },
      { label: "Trends", href: "/health/trends" },
      { label: "Sessions", href: "/health/sessions" },
      { label: "Insights", href: "/health/insights" },
    ],
  },
  {
    label: "Galaxus",
    ...
    >
      <nav className="mx-auto flex h-14 max-w-[1600px] items-center gap-1 px-4">
            ...
            return (
              <div key={group.label} className="group relative">
                ...
                <div className="invisible absolute left-0 top-full z-50 pt-1 opacity-0 transition group-hover:visible group-hover:opacity-100">
                  <div className="min-w-[220px] rounded-xl border border-slate-200 bg-white p-1.5 shadow-lg dark:border-slate-700 dark:bg-slate-900">
                      ...
                      return (
                        <Link
                          ...
                          className={`flex items-center justify-between rounded-lg px-3 py-2 text-sm transition ${
                            active
                              ? "bg-slate-900 text-white dark:bg-[#55b3f3] dark:text-slate-950"
        ...
        <div className="ml-auto flex items-center gap-2">
          <ThemeToggle className="rounded-lg border border-slate-300 px-3 py-1.5 text-sm font-medium text-slate-600 transition hover:bg-slate-100 hover:text-slate-900 dark:border-slate-600 dark:text-slate-300 dark:hover:bg-slate-800 dark:hover:text-white" />
          <button
            onClick={logout}
      ...
      {/* Mobile menu */}
      {mobileOpen ? (
        <div className="max-h-[70vh] overflow-y-auto border-t border-slate-200 bg-white px-4 py-3 dark:border-slate-800 dark:bg-slate-950 md:hidden">
          {NAV_GROUPS.map((group) => {
            const expanded = openGroup === group.label;
            return (
              <div key={group.label} className="border-b border-slate-100 last:border-0 dark:border-slate-800">
                <button
                  type="button"
                  onClick={() => setOpenGroup(expanded ? null : group.label)}
                  className="flex w-full items-center justify-between py-2.5 text-sm font-semibold text-slate-800 dark:text-slate-100"
                >
                  {group.label}
                  <svg
          ...
          <div className="mt-3 flex flex-col gap-2">
            <ThemeToggle className="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm font-medium text-slate-600 hover:bg-slate-100 dark:border-slate-600 dark:text-slate-300 dark:hover:bg-slate-800" />
            <button
              onClick={logout}
              className="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm font-medium text-slate-600 hover:bg-slate-100 dark:border-slate-600 dark:text-slate-300 dark:hover:bg-slate-800"
            >
              Log out
            </button>
          </div>
