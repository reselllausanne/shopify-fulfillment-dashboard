...
import { BeleuchtungdirektClient, bldConfig, type BldProduct } from "@/app/lib/beleuchtungdirektClient";
...
function deferBldImageSync(): boolean {
  return String(process.env.SCRAPER_BLD_DEFER_IMAGE_SYNC ?? "1") !== "0";
}

function parseBldNoteFields(raw: string | null | undefined): { objectId?: string } {
  try {
    return (JSON.parse(String(raw || "")) as { objectId?: string }) || {};
  } catch {
    return {};
  }
}
...
function formatBldNote(product: BldProduct, galaxusKind: string, cost: ReicheltLandedCost) {
  return JSON.stringify({
    type: "beleuchtungdirekt_landed_cost",
    ...
    stockSource: "algolia_in_stock",
  });
}

function runMessage(stats: BldRunStats, imageSynced: number, imageFailed: number) {
  return [
    "source=beleuchtungdirekt-algolia",
    `listed=${stats.listed}`,
    ...
    deferBldImageSync() ? "image_sync=deferred" : "image_sync=inline",
  ].join(" ");
}
...
async function upsertBldVariant(
  prismaAny: any,
  shop: ScraperShop,
  product: BldProduct,
  galaxusKind: string,
  ...
  const queueImage = !deferBldImageSync() && needsImageHosting(existing, product.imageUrl);
  const now = new Date();
  ...
  await prismaAny.supplierVariant.upsert({
    ...
    create: {
      supplierVariantId,
      supplierSku: product.supplierSku ?? product.gtin,
      providerKey,
      gtin: product.gtin,
      ...
      images: [],
      weightGrams: cost.weightGrams,
      manualNote: formatBldNote(product, galaxusKind, cost),
...
export async function scrapeBeleuchtungdirektShop(
  shop: ScraperShop,
  runId: number,
  maxProducts?: number
): Promise<void> {
  const prismaAny = prisma as any;
  const client = new BeleuchtungdirektClient(shop.baseUrl);
  const stats: BldRunStats = {
    listed: 0,
    processed: 0,
    wrote: 0,
    gtinMatched: 0,
  ...
  const deltaDays = Math.max(0, Number(process.env.SCRAPER_BLD_DELTA_DAYS ?? 3));
  const freshCutoffMs = deltaDays > 0 ? Date.now() - deltaDays * 86_400_000 : 0;
  const freshObjectIds = new Set<string>();
  ...
  if (deltaDays > 0) {
    for (const row of existingRows) {
      ...
      const objectId = String(parseBldNoteFields(row.manualNote).objectId || "").trim();
      if (objectId) freshObjectIds.add(objectId);
    }
    console.log(`[SCRAPER] bld delta: skip ${freshObjectIds.size} products synced within ${deltaDays}d`);
  }

  try {
    for await (const product of client.iterProducts(maxProducts)) {
      stats.listed++;
      stats.processed++;

      if (freshObjectIds.has(product.objectId)) {
        stats.skippedFresh++;
        continue;
      }
      if (seenGtins.has(product.gtin)) continue;
      ...
      const ok = await upsertBldVariant(
        prismaAny,
        shop,
        product,
        galaxusKind,
      ...
      if (stats.processed % 50 === 0) {
        await updateRun(runId, {
          ...
          message: runMessage(stats, imageSynced, imageFailed),
        });
      }
      if (!deferBldImageSync() && imageSyncQueue.size >= IMAGE_SYNC_BATCH) {
    ...
    if (stats.wrote > 0) {
      try {
        ...
        console.warn(`[SCRAPER] bld feed push schedule failed:`, (err as Error)?.message || err);
      }
    }

    await updateRun(runId, {
      status: stats.listed === 0 ? "error" : "ok",
      ...
      message:
        stats.listed === 0
          ? `${runMessage(stats, imageSynced, imageFailed)} · bld_unreachable_retry_later`
          : runMessage(stats, imageSynced, imageFailed),
    });
  } catch (err: any) {
    stats.requestErrors++;
    await updateRun(runId, {
