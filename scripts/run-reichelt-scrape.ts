/**
 * Run Reichelt HTML scrape (CLI).
 *
 * Usage:
 *   npx tsx scripts/run-reichelt-scrape.ts
 *   npx tsx scripts/run-reichelt-scrape.ts --max=200
 */
...
async function main() {
  ...
  if (!shop) throw new Error("REI not configured in SCRAPER_SHOPS");

  if (await hasRunningRun(shop.key)) {
    console.log(JSON.stringify({ ok: false, error: "rei scrape already running" }));
    return;
  }

  const runId = await startRun(shop);
  console.log(JSON.stringify({ ok: true, shop: shop.key, runId, maxProducts: maxProducts ?? null }));
  await scrapeReicheltShop(shop, runId, maxProducts);
  const count = await prisma.supplierVariant.count({
    where: { supplierVariantId: { startsWith: "rei_" } },
  });
  console.log(JSON.stringify({ ok: true, runId, reiRows: count }));
}
